...
**ES_CHUNK_NUMBER_GET_LOGS_CLUSTER** - by default 5000, the number of objects which is queried in ES while getting logs from Elasticsearch for clustering. **NOTE**: AWS Elasticsearch has restrictions for sent data size either 10Mb or 100Mb, so when 10Mb is chosen, make sure you don't get the error "TransportError(413, '{"Message": "Request size exceeded 10485760 bytes"}')" while clustering. If you get this error, please, decrease **ES_CHUNK_NUMBER_GET_LOGS_CLUSTER** until you stop getting this error.
