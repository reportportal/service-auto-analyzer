...
def __calculate_hashes(messages: list[str], n_gram_length: int = 2, n_permutations: int = 64) -> list[list[str]]:
    if n_gram_length <= 0:
...
def __unite_groups_by_hashes(messages: list[str], threshold: float = 0.95) -> dict[int, list[int]]:
    start_time = time()
    ...
    if not has_no_empty:
        return {}
    hash_groups: dict[int, int] = __similarity_grouping(hash_prints, for_text=False, threshold=threshold)
...
def __find_groups_by_similarity(messages: list[str], threshold: float = 0.95) -> dict[int, list[int]]:
    if len(messages) == 0:
        return {}
    ...
    for key_word in groups_to_check:
        hash_prints: list[str] = []
        for i in groups_to_check[key_word]:
            hash_prints.append(messages[i])
        hash_groups = __similarity_grouping(hash_prints, for_text=True, threshold=threshold)
...
def __perform_light_deduplication(messages: list[str]) -> tuple[list[str], dict[int, list[int]]]:
    text_messages_set = {}
    messages_to_cluster = []
    ids_with_duplicates = {}
    new_id = 0
