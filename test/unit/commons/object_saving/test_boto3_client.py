...
def test_list_folder_not_recursive(bucket_prefix, bucket):
    ...
    boto3_client = create_storage_client(bucket_prefix)
    boto3_client.put_project_object({"test": True}, bucket, f"{model_folder}/boost_model.pickle", using_json=True)
    boto3_client.put_project_object(
        {"test": True}, bucket, f"{model_folder}/data_features_config.pickle", using_json=True
    )

    assert boto3_client.get_folder_objects(bucket, path) == [model_folder]
