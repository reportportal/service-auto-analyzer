...
def create_test_index_service():
    ...
    return IndexService(DEFAULT_ES_CONFIG, es_client)
...
def test_to_launch_test_item_list(launch, expected_launch):
    index_service = create_test_index_service()
    items = index_service._to_launch_test_item_list([launch])
...
def test_to_index_bodies(launch, expected_length):
    index_service = create_test_index_service()
    test_item_ids, bodies = index_service._to_index_bodies(
        str(TEST_PROJECT_ID), index_service._to_launch_test_item_list([launch])
    )
